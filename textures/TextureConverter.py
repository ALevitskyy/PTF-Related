...
def get_blender():
    ...
    for i in np.unique(vertex_map["faceidx"]):
        ...
        face = {
            "verteces": np.array(mini_map["vertidx"]),
...
class DensePoseMethods:
    ...
    def IUV2FBC(self, I_point, U_point, V_point):
        ...
        P_0 = np.vstack(
            (
                self.U_norm[FacesNow][:, 0],
                self.V_norm[FacesNow][:, 0],
                np.zeros(self.U_norm[FacesNow][:, 0].shape),
            )
        ).transpose()
        P_1 = np.vstack(
            (
                self.U_norm[FacesNow][:, 1],
                self.V_norm[FacesNow][:, 1],
                np.zeros(self.U_norm[FacesNow][:, 1].shape),
            )
        ).transpose()
        P_2 = np.vstack(
            (
                self.U_norm[FacesNow][:, 2],
                self.V_norm[FacesNow][:, 2],
        ...
        if (minD1 < minD2) & (minD1 < minD3):
            return (FaceIndicesNow[0][np.argmin(D1)], 1.0, 0.0, 0.0)
        elif (minD2 < minD1) & (minD2 < minD3):
            return (FaceIndicesNow[0][np.argmin(D2)], 0.0, 1.0, 0.0)
        else:
            return (FaceIndicesNow[0][np.argmin(D3)], 0.0, 0.0, 1.0)
        ...
        for i in range(size_i):
            ...
            for u in range(size_u):
                for v in range(size_v):
                    ...
                    texture[
                        int(UV_coord[0] * 255), int(UV_coord[1] * 255), :
                    ] = atlas_im[i, v, u, :]
...
def precompute_lookup():
    ...
    for i in range(1, 25):
        for x in range(256):
            print(i, x)
            for y in range(256):
                precomputed.append([[i, x, y], methods.IUV2UV(i, x / 255, y / 255)])
