...
def visualize_atlas(atlas):
    ...
    combo = [combine_atlas(atlas[0]),
             combine_atlas(atlas[1])]
    for i in combo:
        plt.imshow(i[:,:,::-1])
        plt.show()
    pass
...
for i in range(1,7000,20): #sample every 20 frames
    ...
    new_atlas1 = get_brother_atlas(init_atlas[0],
                                   new_atlas,
                                   threshold)
    new_atlas2 = get_brother_atlas(init_atlas[1],
                                   new_atlas,
                                   threshold) # get 2 nearest textures
    ...
    init_atlas[mask_both] = 0.8*init_atlas[mask_both]+\
                            0.2*new_atlas[mask_both]
