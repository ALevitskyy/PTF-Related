...
class UNet(nn.Module):
    def __init__(self, basenet="vgg11", num_filters=16, pretrained="imagenet"):
        ...
        self.center = Decoder(
            self.encoder5.out_channels, num_filters * 8 * 2, num_filters * 8
        )

        self.decoder5 = Decoder(
            self.encoder5.out_channels + num_filters * 8,
            num_filters * 8 * 2,
            num_filters * 8,
        )
        self.decoder4 = Decoder(
            self.encoder4.out_channels + num_filters * 8,
            num_filters * 8 * 2,
            num_filters * 4,
        )
        self.decoder3 = Decoder(
            self.encoder3.out_channels + num_filters * 4,
            num_filters * 4 * 2,
            num_filters * 2,
        )
        ...
        if basenet.startswith("vgg"):
            self.decoder2 = Decoder(
                self.encoder2.out_channels + num_filters * 2,
                num_filters * 2 * 2,
                num_filters,
            )
            self.decoder1 = nn.Sequential(
                nn.Conv2d(
                    self.encoder1.out_channels + num_filters,
                    num_filters,
                    kernel_size=3,
                    padding=1,
                ),
            ...
            self.decoder1 = Decoder(
                self.encoder1.out_channels + num_filters, num_filters * 2, num_filters
            )
    ...
    def forward(self, x):
        ...
        logit_pixel = (
            self.logit_pixel1(d1),
            self.logit_pixel2(u2),
            self.logit_pixel3(u3),
            self.logit_pixel4(u4),
...
def symmetric_lovasz_ignore_empty(outputs, targets, truth_image):
    return (
        lovasz_loss_ignore_empty(outputs, targets, truth_image)
        + lovasz_loss_ignore_empty(-outputs, 1 - targets, truth_image)
    ) / 2


def deep_supervised_criterion(
    logit, logit_pixel, logit_image, truth_pixel, truth_image, is_average=True
):
    ...
    for l in logit_pixel:
        loss_pixel += symmetric_lovasz_ignore_empty(
            l.squeeze(1), truth_pixel, truth_image
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--basenet", choices=BASENET_CHOICES, default="vgg11", help="model of basenet"
    )
    parser.add_argument(
        "--num-filters", type=int, default=16, help="num filters for decoder"
    )
    ...
    print(
        "N of encoder parameters {} ({} tensors)".format(
            n_encoder_params, len(encoder_parameters)
        )
    )
    print(
        "N of decoder parameters {} ({} tensors)".format(
            n_params - n_encoder_params, len(parameters) - len(encoder_parameters)
        )
    )
