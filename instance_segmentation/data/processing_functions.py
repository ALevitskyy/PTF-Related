...
def outlines(img):
    # Get boundaries
    return cv2.morphologyEx(img, cv2.MORPH_GRADIENT, np.ones((5, 5), np.uint8))


def merge_human(mask1, mask2):
    # Self explanatory
    combo = mask1 + mask2
    smoothed = cv2.morphologyEx(combo, cv2.MORPH_CLOSE, np.ones((2, 2), np.uint8))
    return smoothed
...
def masks2targets(maski):
    ...
    targets[0][targets[1] > 0] = 0
    return np.array(targets, np.uint8)


def color2targets(color_image):
    masks = get_masks(color_image)
    targets = masks2targets(masks)
    return targets
...
def get_components(merged, targets):
    ...
    n_comp, labels = cv2.connectedComponents(merged.astype(np.uint8))
    smooth_labels = cv2.morphologyEx(
        labels.astype(np.uint8), cv2.MORPH_DILATE, np.ones((7, 7), np.uint8)
    )
...
def is_neighbor(comp1, comp2):
    ...
    comp1_dil = cv2.morphologyEx(
        comp1.astype(np.uint8), cv2.MORPH_DILATE, np.ones((2, 2), np.uint8)
    )
...
def assign_labels(labels):
    ...
    while len(processed_queue) != 0:
        ...
        for i in left:
            if is_neighbor(
                (labels == label).astype(np.uint8), (labels == i).astype(np.uint8)
            ):
    ...
    for i in left:
        ...
        for key in keys:
            distances.append(minim_dist(get_box(labels == i), get_box(labels == key)))
...
def output2final(classes):
    targets = np.eye(6)[classes].transpose(2, 0, 1)
    humans = merge_boundaries(targets)
    smooth_labels = get_components(humans, targets)
    assigned = assign_labels(smooth_labels)
    targets[0] = (assigned == 1).astype(np.uint8)
    targets[1] = (assigned == 2).astype(np.uint8)
